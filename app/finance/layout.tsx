...
export default function FinanceLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: '/finance/kash', label: 'K-Kash' },
    ...
    <div className="min-h-screen bg-surface text-slate-200">
      ...
      <aside className="fixed left-0 top-0 h-full w-64 bg-surface-1 border-r border-surface-border z-40">
        <div className="px-6 py-5 border-b border-surface-border">
          <Link href="/finance" className="font-display font-bold text-xl text-amber-400">
            KOLI
          </Link>
          <p className="text-xs text-slate-500 mt-1">Finance Console</p>
        </div>
      ...
      <div className="pl-64 relative">
        <header className="h-16 bg-surface-1/80 backdrop-blur-sm border-b border-surface-border sticky top-0 z-30 px-6 flex items-center justify-between">
          <div>
            <h1 className="font-display text-lg text-slate-100">Finance</h1>
            <p className="text-xs text-slate-500">Treasury operations and approvals</p>
          </div>
